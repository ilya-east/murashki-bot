fetch('tracks.json')
  .then(res => res.json())
  .then(data => {
    const container = document.getElementById('player-column');
    data.forEach(track => {
      const player = document.createElement('div');
      player.className = 'custom-player';
      player.innerHTML = `
        <img class="cover" src="covers/${track.image}" alt="cover">
        <div class="player-info">
          <div class="player-title">${track.title}</div>
          <div class="player-author">${track.artist}</div>
        </div>
        ...
        <audio src="audio/${track.audio}"></audio>
      `;
